...
QNNExecutionProvider::QNNExecutionProvider(const ProviderOptions& provider_options_map,
  ...
  if ((context_cache_enabled_ || enable_vtcm_backup_buffer_sharing_)
       && share_ep_contexts_
...
QNNExecutionProvider::GetCapability(const onnxruntime::GraphViewer& graph_viewer,
  ...
  // buffer_file_map needed to persist buffer during context creation
  std::unordered_map<std::string, std::vector<unsigned char>> buffer_file_map;
  ...
  if (enable_vtcm_backup_buffer_sharing_) {
    ...
    for (auto& ep_ctx_node : ep_ctx_nodes) {
      ...
      auto context_bin_filepath = node_helper.Get(qnn::EP_CACHE_CONTEXT, "");

      if (buffer_file_map.find(context_bin_filepath) != buffer_file_map.end()) {
        continue;
      }

      std::ifstream binary_in(context_bin_filepath, std::ios::binary);

      if (binary_in.fail()) {
        LOGS(logger, ERROR) << "Unable to retrieve context bin at \""
                            << context_bin_filepath << "\" for "
                            << ep_ctx_node->Name();
        return result;
      }

      std::vector<unsigned char> buffer(std::istreambuf_iterator<char>(binary_in), {});
      buffer_file_map.emplace(context_bin_filepath, std::move(buffer));
    }
  }
